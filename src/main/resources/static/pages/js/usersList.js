...
var List = function () {
	...
	var handleList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            datatype: "json",  
            colNames: ['ID', '头像', '用户名', '性别', '手机号', '状态', '登陆日期', '操作'],
            colModel: [  
                { name: 'openid', index: 'openid', width: 30, sortable: false, hidden: false },
                { name: 'faceImage', index: 'faceImage', width: 30, sortable: false,
                ...
                { name: 'phone', index: 'phone', width: 20, sortable: false },
                { name: 'state', index: 'state', width: 20, sortable: false },
                { name: 'create_time', index: 'create_time', width: 20, sortable: false },
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                    formatter:function(cellvalue, options, rowObject) {
                        var button = '<button class="btn btn-outline blue-chambray" id="" onclick=forbidVideo("' + rowObject.dealVideoId + '") style="padding: 1px 3px 1px 3px;">通过</button>';
                        return button;
                    }
                }
            ],  
